...
# --- Função para encontrar arquivos no modo .exe ---
...
def save_config():
    ...
    # Se o tema atual for um easter egg, salva o tema padrão correspondente
    if current_theme == 'vapor':
        theme_to_save = 'darkly'
    elif current_theme == 'solar':
        theme_to_save = 'litera'
...
def activate_theme(theme_name, message, is_light):
    """Função genérica para ativar um tema de easter egg."""
...
def vapor_easter_egg_check(event):
    ...
    vapor_clicks += 1
    if vapor_clicks >= 7:
        activate_theme('vapor', "Tema Vaporwave Ativado!", is_light=False)
        vapor_clicks = 0
...
def solar_easter_egg_check(event):
    ...
    solar_clicks += 1
    if solar_clicks >= 7:
        activate_theme('solar', "Tema Solar Ativado!", is_light=True)
        solar_clicks = 0
...
def automation_worker(sequence_name, matriculas_lista, periodo):
    ...
    try:
        ...
        if session is None:
            raise ConnectionError("Falha na conexão com o SAP.")
        ...
        for i, processo_nome in enumerate(tarefas_finais_para_executar):
            ...
            if processo_nome in PROCESS_MAP:
                ...
                sucesso, mensagem = funcao_a_executar(session, matriculas_lista, periodo, {}, output_base_path, progress_queue=progress_queue)
                if not sucesso:
...
def start_automation(sequence_name):
    ...
    progress_list.delete(*progress_list.get_children())
    progress_list_items.clear()
    
    automation_thread = threading.Thread(target=automation_worker, args=(sequence_name, matriculas_lista, periodo), daemon=True)
    automation_thread.start()
    app.after(100, process_queue)

def process_queue():
    """Verifica a fila e atualiza a UI. Roda na thread principal."""
    try:
        while True:
            ...
            if msg["type"] == "status":
                ...
                geral_status_var.set("Erro!")
                detalhe_status_var.set(msg["msg"][:100] + "...")
...
if __name__ == "__main__":
    ...
    app.state('zoomed') 
    app.minsize(width=900, height=700)
    ...
    exec_frame.bind("<Button-1>", solar_easter_egg_check) # Easter Egg do Solar
    ...
    params_frame = ttk.Labelframe(center_frame, text=" 2. Entradas ", padding=10)
    ...
    progress_frame = ttk.Labelframe(center_frame, text=" 3. Progresso Detalhado ", padding=10)
