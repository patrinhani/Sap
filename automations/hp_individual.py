import time
import os
# A biblioteca 'calendar' não é mais necessária aqui
...
def execute(session, matriculas, periodo, options, base_path):
    ...
    caminho_vba = pasta_data # Salva na raiz da pasta da data
    if not os.path.exists(caminho_vba):
        os.makedirs(caminho_vba)

    # --- CORREÇÃO AQUI: Lógica de data alterada para ser fixa, como no VBA ---
    # O script agora ignora o parâmetro 'periodo' e usa estas datas fixas.
    ...
    try:
        for matricula in matriculas:
            ...
            grid.selectColumn("OCRTX"); grid.contextMenu(); grid.selectContextMenuItem("&FILTER"); time.sleep(1)
            ...
            grid.selectColumn("PAYDT"); grid.contextMenu(); grid.selectContextMenuItem("&FILTER"); time.sleep(1)
            
            # Usa as datas fixas que definimos no início
