...
def execute(session, matriculas, periodo, config, output_base_path, progress_queue=None):
  ...
  # --- 2028 ---
    ...
    ('12', '2028'),
        ...
        meses_a_processar = [m for m in iterar_meses(periodo['inicio'], periodo['fim']) if (m[0], m[1]) in datas_validas]
        if progress_queue:
            progress_queue.put({"type": "task_list", "tasks": [f"ZDP2 - {mes}/{ano}" for mes, ano in meses_a_processar]})
        ...
        for i, (mes, ano) in enumerate(meses_a_processar):
            ...
            caminho_de_saida = get_save_path(output_base_path, "ZDP2", ano=ano, mes=mes)
