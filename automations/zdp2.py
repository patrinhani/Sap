...
def execute(session, matriculas, periodo, config, output_base_path):
    """Executa a automação para a variante ZDP2."""
    try:
        print("--- Iniciando processo 'ZDP2' ---")
        ...
        if not inserir_matriculas(session, matriculas):
            return False, "Falha ao inserir matrículas no processo ZDP2."
        time.sleep(1)

        for mes, ano in iterar_meses(periodo['inicio'], periodo['fim']):
            if (mes, ano) in datas_validas:
                print(f"Processando ZDP2 para data válida: {mes}/{ano}...")
                
                session.findById("wnd[0]/usr/radPNPTIMRA").select()
                session.findById("wnd[0]/usr/ctxtBONDT").text = ""
                ...
                print(f"Ignorando data {mes}/{ano} para ZDP2 (não está na lista).")

        print("--- Processo 'ZDP2' finalizado. ---")
        return True, "Processo ZDP2 concluído com sucesso."

    except Exception as e:
        print(f"ERRO no processo ZDP2: {e}")
        return False, f"Erro no processo ZDP2: {e}"
