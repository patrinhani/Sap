...
def execute(session, matriculas, periodo, options, base_path):
    """
    Executa o PLR 2022. Ignora o período da interface e usa valores fixos.
    """
    ...
    try:
        ...
        time.sleep(1)
        pyperclip.copy("\n".join(matriculas))
        session.findById("wnd[1]/tbar[0]/btn[24]").press()  # Botão "Importar da área de transferência"
        time.sleep(0.5)
        session.findById("wnd[1]/tbar[0]/btn[8]").press()  # Botão "Executar" (Confirmar seleção)
        time.sleep(1)
        ...
        return False, f"Erro em PLR 2022: {e}"