...
def execute(session, matriculas, mes, ano, output_base_path, progress_queue=None):
    if not matriculas: return True, "Nenhuma matrícula para ZCT1."
    ...
    try:
        ...
        # A lógica de inserção de matrículas aqui pode precisar ser ajustada
        # dependendo se esta variante aceita múltiplas matrículas ou apenas uma.
        # Assumindo que aceita múltiplas por enquanto.
        from .hp import inserir_matriculas # Reutiliza a função robusta
        if not inserir_matriculas(session, matriculas):
            raise RuntimeError("Falha ao inserir matrículas no processo ZCT1.")
        
        keep_alive(session)
        start_sapgui_security_watcher(timeout=10)
        session.findById("wnd[0]/tbar[1]/btn[8]").press(); time.sleep(2)
        session.findById("wnd[0]/tbar[0]/btn[15]").press(); time.sleep(1) # Volta para o menu principal
