...
def execute(session, matriculas, periodo, options, base_path):
    """
    Executa a 2ª parcela do 13º. Usa o período de anos da interface
    e calcula a Data BONDT internamente com a regra específica do ano.
    """
    print("--- Iniciando execução de HP 13.2 ---")
    try:
    ...
    try:
        ...
        grid.setCurrentCell(1, "TEXT")  # Variante 13. Sal. 2ª Parc
        grid.selectedRows = "1"
        ...
        for ano_atual in range(ano_inicio, ano_fim + 1):
            ...
            dia_bondt = "13" if ano_atual == 2024 else "15"
            data_bondt_final = f"{dia_bondt}.12.{ano_atual}"
            print(f"    Usando Data BONDT fixa: {data_bondt_final}")

            session.findById("wnd[0]/usr/txtPNPPABRP").text = "12" # Mês fixo
        ...
        return True, "Processo HP 13.2 concluído."
    except Exception as e:
        return False, f"Erro em HP 13.2: {e}"