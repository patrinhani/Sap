...
def execute(session, matriculas, periodo, options, base_path):
    print("--- Iniciando execução de ZDP1 ---")
    try:
        mes_inicio, ano_inicio = 8, 2024
        mes_fim, ano_fim = int(periodo['fim'].split('/')[0]), int(periodo['fim'].split('/')[1])
        if (ano_fim < 2024) or (ano_fim == 2024 and mes_fim < 8): return False, "Erro: Período final para ZDP1 deve ser a partir de Agosto/2024."
    ...
    try:
        ...
        while (ano_atual, mes_atual) <= (ano_fim, mes_fim):
            ...
            if primeira_vez_variante:
                ...
                print("  Selecionando variante 'ZDP1' via busca...")
            ...
            if not data_bondt:
                print(f"    AVISO: Data BONDT não encontrada para {periodo_str}. Campo ficará vazio.")
                 ...
                 # O VBA de ZDP1 não tinha a linha para preencher o BONDT, mas se for necessário, descomente abaixo:
        ...
        return True, "Processo ZDP1 concluído."
    except Exception as e: return False, f"Erro em ZDP1: {e}"